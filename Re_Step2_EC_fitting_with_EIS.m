...
yyaxis left;
hold on;
plot(ratios, specific_power_30s, 'ob', 'MarkerFaceColor', 'b', 'DisplayName', '30s Short-term');
...
yyaxis right;
plot(ratios, specific_power_30min, 'sr', 'MarkerFaceColor', 'r', 'DisplayName', '30min Long-term');
...
Rs = fit_params_all(:, 1);
Qdl = fit_params_all(:, 2);
n = fit_params_all(:, 3);
Rct1 = fit_params_all(:, 4);
Rw1 = fit_params_all(:, 5);
Rct2 = fit_params_all(:, 7);
Rw2 = fit_params_all(:, 8);
Cdl = (Qdl .* (Rct1 .^ (1 - n))) .^ (1 ./ n);